...
class HydricRecommend:
    def recommend(self, user_id, kind='NFM', k = 5):
        if kind == 'NFM':
            embedding = torch.load(BASE_PATH + '/recommender/nfm_rec_data.pt')
        elif kind == 'LGN':
            embedding = torch.load(BASE_PATH + '/recommender//lgc_rec_data.pt')

        # user_id = embedding['user_id_map'].get(user, '')
        df = pd.read_csv(f"{BASE_PATH}/database/user_place.csv")
        ...
        user_emb = embedding['user_emb'][int(user_id)]
        scores = torch.matmul(item_emb, user_emb)
        topk_idx = torch.topk(scores, k).indices

        return [str(int(idx)) for idx in topk_idx]
